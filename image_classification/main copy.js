...
async function renderCamStream() {
  ...
  await netInstance.computeGPUTensor(inputGPUBuffer, outputBuffer); // computeGPUTensor compute
...
async function main() {
  try {
    ...
    if (inputType === 'image') {
      const inputData = utils.getInputTensor(imgElement, inputOptions);
      const inputGPUBuffer = await utils.createGPUBuffer(netInstance.device_, utils.sizeOfShape(inputOptions.inputDimensions), inputData); 
      console.log('- Computing... ');
      ...
      if (numRuns > 1) {
        // Do warm up compute
        await netInstance.computeGPUTensor(inputGPUBuffer, outputBuffer); // computeGPUTensor
      }
      // if (inputBuffer instanceof tf.Tensor) {
      //   inputBuffer.dispose();
      // }
      for (let i = 0; i < numRuns; i++) {
        const inputData = utils.getInputTensor(imgElement, inputOptions);
        const inputGPUBuffer = await utils.createGPUBuffer(netInstance.device_, utils.sizeOfShape(inputOptions.inputDimensions), inputData); 
        start = performance.now(); // netInstance.computeGPUTensor
        await netInstance.computeGPUTensor(inputGPUBuffer, outputBuffer); // computeGPUTensor compute
        computeTime = (performance.now() - start).toFixed(2);
